...
- **Sentences:** full_text must contain **at most 5 sentences** (complete thoughts ending with `.`, `?`, or `!` appropriate to `{target_language}`; do not split one idea into extra sentences to bypass this).
- **Gaps — count:** `hard_words` must contain **at most 10 entries** (at most **10 words** appear as gaps). If `registered_hard_words` lists more than 10 items, create gaps for **at most 10** of them — pick a subset that fits naturally; you may leave other listed words in `full_text` as normal (non-gap) text, or omit them if they cannot fit without exceeding limits.
...
- **Training vocabulary (primary):** `registered_hard_words` are the **words to practise**. Prefer each **verbatim** in `full_text`; up to **10** of them may be **gaps** (`hard_words`). If there are **10 or fewer** listed words, gap **all** of them (one entry each unless duplicate surfaces — see below). If there are **more than 10**, gap **at most 10** chosen for natural fit; others may appear as plain text or be omitted if limits forbid.
- **Extra gaps (rare):** Only if you **cannot** include a listed word after a reasonable rewrite, you may replace **that single slot** with a minimal alternative gap word still in `{target_language}` — still respect **at most 10** `hard_words` total. Keep such exceptions **few**.
- **Duplicates:** If the same surface appears **more than once** in `full_text` and each occurrence should be a gap, include **one hard_words entry per occurrence** (same `surface`, distinct id / span) **only if** the total number of `hard_words` entries stays **≤ 10**.
    ...
    json!({
        ...
        "passage_authoring": {
            ...
            "length_limits": {
                "max_sentences": 5,
                "max_gap_words": 10,
                "note": "At most 5 sentences in full_text; at most 10 hard_words entries. If registered_hard_words is longer than 10, gap only up to 10 of them."
            },
            "must_not": [
                ...
                "Exceed 5 sentences in full_text or more than 10 gap words in hard_words.",
                "Produce disconnected or random sentences that do not read as one flowing story."
            ],
            "must": [
                "Keep full_text to at most 5 sentences and hard_words to at most 10 entries (see system message).",
                ...
                "If registered_hard_words has 10 or fewer items, prefer gapping all of them; if more than 10, gap at most 10 and choose the best-fitting subset.",
                "Include only substrings of full_text in hard_words; keep full_text within the max word count from the system message."
            ],
            "exceptions": "When limits conflict with the full vocabulary list, respect the 5-sentence and 10-gap caps first; only when a listed word truly cannot fit within those caps, omit that gap or use a rare substitute per system rules."
        }
    })
}
